    ...
    def __getitem__(self, idx):
        ...
        if end - start < self.window:
            ...
            act = utils.inference.repeat_start_to_length(
                act[start : end - 1 + self.action_window], # -1 due to overlap for 1 step between obs and act
            ...
            act = act[start : end - 1 + self.action_window] # -1 due to overlap for 1 step between obs and act
        ...
        if act.shape[0] < expected_len:
            if self.pad_seq_length:
                act = utils.inference.repeat_end_to_length(
                    act, expected_len, dim=0
                )
            else:
                raise ValueError(
        ...
        # print(" | ".join(str(v.shape) for v in values))
        return tuple(values)


# old version before bugfix
...
#             values.append(torch.ones([1, 1, 1, 1]))
#         return tuple(values)
